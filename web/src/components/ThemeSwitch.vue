<template>
  <v-theme-provider
    class="container"
    :theme="check ? 'dark' : 'auto'"
    with-background
  >
    <slot></slot>
  </v-theme-provider>
  <label id="theme-toggle-button" class="theme-switch">
    <input type="checkbox" id="toggle" v-model="check" @change="handleChange" />
    <svg
...
const THEME_STORAGE_KEY = "theme_appearance";
const themeStorage = storage.get(THEME_STORAGE_KEY) || "auto";

const check = ref<boolean>(themeStorage === "dark");
const handleChange = (e: Event) => {
  const dark = (<HTMLInputElement>e.target).checked as boolean;
  check.value = dark;
  storage.set(THEME_STORAGE_KEY, dark ? "dark" : "auto");
};
</script>
